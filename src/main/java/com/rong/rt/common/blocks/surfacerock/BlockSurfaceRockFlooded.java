package com.rong.rt.common.blocks.surfacerock;
...
public class BlockSurfaceRockFlooded extends BlockLiquid {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if(!worldIn.isAreaLoaded(pos, 4)) return; // Forge: avoid loading unloaded chunks
	...
	private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos) {
		...
		for(EnumFacing direction : EnumFacing.Plane.HORIZONTAL) {
			...
			if(!isBlockBlockedForFlow(blockState)
					&& (blockState.getMaterial() != blockMaterial || blockState.getValue(LEVEL) > 0)) {
				int slopeDistance;
	...
	private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
		...
		for(EnumFacing direction : EnumFacing.Plane.HORIZONTAL) {
			if(direction != calculateFlowCost) {
				...
				IBlockState blockState = worldIn.getBlockState(offsetPos);
				if(!isBlockBlockedForFlow(blockState)
						&& (blockState.getMaterial() != this.blockMaterial || blockState.getValue(LEVEL) > 0)) {
					if(!isBlockBlockedForFlow(worldIn.getBlockState(offsetPos.down()))) {
						return distance;
					}
					if(distance < 4) {
						int thisSlopDistance = this.getSlopeDistance(worldIn, offsetPos, distance + 1,
	...
	private boolean canFlowInto(IBlockState state) {
		...
		return material != this.blockMaterial && material != net.minecraft.block.material.Material.LAVA
				&& !isBlockBlockedForFlow(state);
	}
	...
	protected BlockStateContainer createBlockState() {
		if(materialProperty == null) return new BlockStateContainer(this, LEVEL);
	...
	private ItemStack getDropStack(IBlockState state, int amount) {
		...
		if(material instanceof IngotMaterial && ((IngotMaterial) material).blastFurnaceTemperature == 0)
			return OreDictUnifier.get(EnumOrePrefix.nugget, material, amount);
		return OreDictUnifier.get(EnumOrePrefix.dustTiny, material, amount);
	}
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		...
		if(fromPos.up().equals(pos)) {
			if(worldIn.getBlockState(fromPos).getBlockFaceShape(worldIn, fromPos,
