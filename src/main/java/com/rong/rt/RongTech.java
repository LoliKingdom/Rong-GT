...
import com.rong.rt.common.GuiHandler;
...
@Mod(modid = Values.MOD_ID, name = Values.MOD_NAME, acceptedMinecraftVersions = "[1.12]", dependencies = "after:ic2;after:ic2-classic-spmod;after:forestry;after:jei@[4.8.6,);after:crafttweaker;")
public class RongTech {
	...
	public void onInit(FMLInitializationEvent event) {
		proxy.onLoad();
		NetworkRegistry.INSTANCE.registerGuiHandler(RongTech.instance, new GuiHandler());
		
		/*
		if(RecipeMap.isFoundInvalidRecipe()) {
			RTLog.logger.fatal("Seems like invalid recipe was found.");
			// crash if config setting is set to false, or we are in deobfuscated
			// environment
	...
	public void onPostInit(FMLPostInitializationEvent event) {
		proxy.onPostLoad();
		// if(Values.isModLoaded("nuclearcraft")) {
		// GTRadSources.init();
		// }
