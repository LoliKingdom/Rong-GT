...
public class ElectricItem implements IElectricItem, ICapabilityProvider {
	...
	public long getMaxCharge() {
		...
		if(tagCompound == null) return maxCharge;
		if(tagCompound.hasKey("MaxCharge", NBT.TAG_LONG)) return tagCompound.getLong("MaxCharge");
		return maxCharge;
	}
	...
	public long charge(long amount, int chargerTier, boolean ignoreTransferLimit, boolean simulate) {
		...
		if((chargeable || amount == Long.MAX_VALUE) && (chargerTier >= tier) && amount > 0L) {
			...
			if(!ignoreTransferLimit) {
				amount = Math.min(amount, Values.V[tier]);
			}
	...
	public boolean hasCapability(@Nonnull Capability<?> capability, @Nullable EnumFacing facing) {
		return capability == RTCapabilities.CAPABILITY_ELECTRIC_ITEM;
	}

	@Nullable
	@Override
	public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
		return capability == RTCapabilities.CAPABILITY_ELECTRIC_ITEM ? (T) this : null;
	}
}
