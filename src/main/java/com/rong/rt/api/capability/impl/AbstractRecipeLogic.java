...
import com.rong.rt.api.capability.IWorkable;
...
public abstract class AbstractRecipeLogic extends MTETrait implements IWorkable {
	...
	public <T> T getCapability(Capability<T> capability) {
		if(capability == RTCapabilities.CAPABILITY_WORKABLE) {
			return RTCapabilities.CAPABILITY_WORKABLE.cast(this);
		}
		else if(capability == RTCapabilities.CAPABILITY_CONTROLLABLE) {
	...
	protected int getByproductChanceMultiplier(Recipe recipe) {
		...
		int recipeTier = Utility.getTierByVoltage(recipe.getEUt());
