...
import com.rong.rt.api.metatileentity.MTETrait;
...
public class EnergyContainerHandler extends MTETrait implements IEnergyContainer {
    ...
    public <T> T getCapability(Capability<T> capability) {
        if(capability == RTCapabilities.CAPABILITY_ENERGY_CONTAINER) {
            return RTCapabilities.CAPABILITY_ENERGY_CONTAINER.cast(this);
        }
        return null;
    }
    ...
    public boolean dischargeOrRechargeEnergyContainers(IItemHandlerModifiable itemHandler, int slotIndex) {
        ...
        IElectricItem electricItem = stackInSlot.getCapability(RTCapabilities.CAPABILITY_ELECTRIC_ITEM, null);
        if(electricItem == null || !electricItem.canProvideChargeExternally()) {
            return false;
        }
        int machineTier = Utility.getTierByVoltage(Math.max(getInputVoltage(), getOutputVoltage()));
    ...
    public void update() {
        ...
        if(getCurrentEnergyStored() >= getOutputVoltage() && getOutputVoltage() > 0 && getOutputAmperage() > 0) {
            ...
            for(EnumFacing side : EnumFacing.VALUES) {
                ...
                if(tileEntity != null && tileEntity.hasCapability(RTCapabilities.CAPABILITY_ENERGY_CONTAINER, oppositeSide)) {
                    IEnergyContainer energyContainer = tileEntity.getCapability(RTCapabilities.CAPABILITY_ENERGY_CONTAINER, oppositeSide);
    ...
    public long acceptEnergyFromNetwork(EnumFacing side, long voltage, long amperage) {
        ...
        if(voltage > 0L && amperage > 0L && (side == null || inputsEnergy(side))) {
            if(voltage > getInputVoltage()) {
                Utility.doOvervoltageExplosion(metaTileEntity, voltage);
