...
import com.rong.rt.api.capability.IControllable;
...
public class FuelRecipeLogic extends MTETrait implements IControllable {
	...
	public FuelRecipeLogic(MetaTileEntity metaTileEntity, FuelRecipeMap recipeMap,
	...
	public <T> T getCapability(Capability<T> capability) {
		if(capability == RTCapabilities.CAPABILITY_CONTROLLABLE) {
			return RTCapabilities.CAPABILITY_CONTROLLABLE.cast(this);
		}
		return null;
	}
	...
	public void update() {
		...
		if(workingEnabled) {
			if(recipeDurationLeft > 0) {
				if(energyContainer.get().getEnergyCanBeInserted() >= recipeOutputVoltage
	...
	/**
	 * Performs preparations for starting given recipe and determines it's output
	 * voltage
	 *
	...
	public static long getTieredVoltage(long voltage) {
		return Values.V[Utility.getTierByVoltage(voltage)];
	}
