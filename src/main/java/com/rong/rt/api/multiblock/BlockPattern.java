package com.rong.rt.api.multiblock;
...
import com.rong.rt.api.utils.IntRange;
...
public class BlockPattern {
	...
	public BlockPattern(Predicate<BlockWorldState>[][][] predicatesIn,
			...
			List<Predicate<PatternMatchContext>> validators, RelativeDirection[] structureDir,
			...
			loop: for(r = 0; (findFirstAisle ? r < aisleRepetitions[c][1] : z <= -centerOffset[3]); r++) {
				...
				for(int b = 0, y = -centerOffset[1]; b < this.thumbLength; b++, y++) {
					for(int a = 0, x = -centerOffset[0]; a < this.palmLength; a++, x++) {
						...
						blockPos.setPos(blockPos.getX() + centerPos.getX(), blockPos.getY() + centerPos.getY(),
	...
	public enum RelativeDirection {
		UP(f -> EnumFacing.UP), DOWN(f -> EnumFacing.DOWN), LEFT(EnumFacing::rotateYCCW), RIGHT(
				EnumFacing::rotateY), FRONT(Function.identity()), BACK(EnumFacing::getOpposite);
