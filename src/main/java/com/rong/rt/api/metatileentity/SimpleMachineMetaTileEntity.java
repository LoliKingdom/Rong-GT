...
import com.rong.rt.api.gui.widgets.DischargerSlotWidget;
...
public class SimpleMachineMetaTileEntity extends WorkableTieredMetaTileEntity {
	...
	public SimpleMachineMetaTileEntity(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap,
			OrientedOverlayRenderer renderer, int tier, boolean hasFrontFacing) {
	...
	public <T> T getCapability(Capability<T> capability, EnumFacing side) {
		if(capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
			IFluidHandler fluidHandler = (side == getOutputFacing() && !allowInputFromOutputSide) ? outputFluidInventory
					: fluidInventory;
			...
			IItemHandler itemHandler = (side == getOutputFacing() && !allowInputFromOutputSide) ? outputItemInventory
					: itemInventory;
	...
	protected ModularUI.Builder createGuiTemplate(EntityPlayer player) {
		ModularUI.Builder builder = workable.recipeMap
				.createUITemplate(workable::getProgressPercent, importItems, exportItems, importFluids, exportFluids)
				.widget(new LabelWidget(5, 5, getMetaFullName()))
				.widget(new DischargerSlotWidget(chargerInventory, 0, 79, 62).setBackgroundTexture(GuiTextures.SLOT,
						GuiTextures.CHARGER_OVERLAY))
		...
		if(exportItems.getSlots() > 0) {
			builder.widget(new ToggleButtonWidget(leftButtonStartX, 62, 18, 18, GuiTextures.BUTTON_ITEM_OUTPUT,
					this::isAutoOutputItems, this::setAutoOutputItems));
			leftButtonStartX += 18;
		}
		if(exportFluids.getTanks() > 0) {
			builder.widget(new ToggleButtonWidget(leftButtonStartX, 62, 18, 18, GuiTextures.BUTTON_FLUID_OUTPUT,
		...
		builder.widget(new ToggleButtonWidget(rightButtonStartX, 60, 20, 20, GuiTextures.BUTTON_OVERCLOCK,
				workable::isAllowOverclocking, workable::setAllowOverclocking)
