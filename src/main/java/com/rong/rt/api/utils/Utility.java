...
import static com.rong.rt.Values.V;
...
public class Utility {
	...
	public static <T, R> Class<T> getActualTypeParameter(Class<? extends R> thisClass, Class<R> declaringClass,
			int index) {
	...
	public static PotionEffect copyPotionEffect(PotionEffect sample) {
		PotionEffect potionEffect = new PotionEffect(sample.getPotion(), sample.getDuration(), sample.getAmplifier(),
				sample.getIsAmbient(), sample.doesShowParticles());
	...
	/**
	 * Attempts to merge given ItemStack with ItemStacks in slot list supplied If
	 * it's not possible to merge it fully, it will attempt to insert it into first
	 * empty slots
	 */
	public static boolean mergeItemStack(ItemStack itemStack, List<Slot> slots) {
		if(itemStack.isEmpty()) return false; // if we are merging empty stack, return

		boolean merged = false;
		// iterate non-empty slots first
		// to try to insert stack into them
		for(Slot slot : slots) {
			if(!slot.isItemValid(itemStack)) continue; // if itemstack cannot be placed into that slot, continue
			...
			if(amountToInsert == 0) continue; // if we can't insert anything, continue
			...
			slot.onSlotChanged();
			merged = true;
			if(itemStack.isEmpty()) return true; // if we inserted all items, return
		}
		...
		for(Slot slot : slots) {
			if(!slot.isItemValid(itemStack)) continue; // if itemstack cannot be placed into that slot, continue
			if(slot.getHasStack()) continue; // if slot contains something, continue
	...
	public static boolean harvestBlock(World world, BlockPos pos, EntityPlayer player) {
		...
		if(!world.isRemote) {
			...
			if(expToDrop == -1) {
				// notify client if block can't be removed because of BreakEvent cancelled on
				// server side
		...
		boolean wasRemovedByPlayer = blockState.getBlock().removedByPlayer(blockState, world, pos, player,
		...
		if(!world.isRemote) {
			...
			mc.getConnection()
					.sendPacket(new CPacketPlayerDigging(Action.START_DESTROY_BLOCK, pos, mc.objectMouseOver.sideHit));
		}
	...
	/**
	 * Applies specific amount of damage to item, either to durable items (which
	 * implement IDamagableItem) or to electric items, which have capability
	 * IElectricItem Damage amount is equal to EU amount used for electric items
	 * 
	...
	public static boolean doDamageItem(ItemStack itemStack, int vanillaDamage, boolean simulate) {
		...
		if(item instanceof IToolItem) {
			...
			if(!simulate && itemStack.attemptDamageItem(vanillaDamage, new Random(), null)) {
				...
				// actually we would play broken animation here, but we don't have an entity who
				// holds item
				itemStack.shrink(1);
			}
			return true;
	...
	public static String capitalizeString(String string) {
		if(string != null && string.length() > 0) return string.substring(0, 1).toUpperCase() + string.substring(1);
		return "";
	}
	...
	public static byte getTierByVoltage(long voltage) {
		...
		while(++tier < V.length) {
			// For conductor
			/*
			 * if(voltage == Integer.MAX_VALUE) { return tier = 6; }
			 */
	...
	public static List<Tuple<ItemStack, Integer>> getGrassSeedEntries() {
		...
		try {
			...
			for(WeightedRandom.Item seedEntryObject : seedList) {
				...
				if(!seedStack.isEmpty()) result.add(new Tuple<>(seedStack, chanceValue));
			}
		} catch(ReflectiveOperationException exception) {
			RTLog.logger.error("Failed to get forge grass seed list", exception);
		}
		return result;
	}
	...
	/**
	 * @return a list of itemstack linked with given item handler modifications in
	 *         list will reflect on item handler and wise-versa
	 */
	...
	public static List<FluidStack> fluidHandlerToList(IMultipleTanksHandler fluidInputs) {
		...
		return new AbstractList<FluidStack>() {
			...
			public FluidStack set(int index, FluidStack element) {
				...
				if(!(fluidTank instanceof FluidTank)) return oldStack;
	...
	public static <T extends Comparable<T>> IBlockState[] getAllPropertyValues(IBlockState blockState,
	...
	/**
	 * Identical to {@link InventoryHelper#dropInventoryItems}, except being
	 * designed for {@link IItemHandler}.
	 * 
	 * @param worldIn
	 *            The world that block is in
	 * @param pos
	 *            The position of block
	 * @param inv
	 *            An array of IItemHandler implementation. Note: all IItemHandler
	 *            here are assumed to start index from zero.
	 */
	public static void dropInventoryItems(World worldIn, BlockPos pos, IItemHandler... inv) {
		for(IItemHandler invSingle : inv) {
			...
			for(int index = 0; index < slots; index++) {
				...
				if(!stack.isEmpty()) dropItemStackAsEntityInsanely(worldIn, pos, stack);
			}
		}
	}

	/**
	 * Identical to {@link InventoryHelper#spawnItemStack} in terms of
	 * functionality, except that this one has insane implementation.
	 * 
	 * @param worldIn
	 *            The world that item stack will drop.
	 * @param pos
	...
	public static <M, E extends M> E selectItemInList(int index, E replacement, List<? extends M> list,
			Class<E> minClass) {
		if(list.isEmpty()) return replacement;
