...
public class FluidTankList implements IFluidHandler, IMultipleTanksHandler, INBTSerializable<NBTTagCompound> {
	...
	private int fillTanksImpl(FluidStack resource, boolean doFill) {
		...
		for(IFluidTank handler : fluidTanks) {
			if(resource.isFluidEqual(handler.getFluid())) {
				...
				if(!allowSameFluidFill || resource.amount == 0) return totalFilled;
			}
		}
	...
	public FluidStack drain(int maxDrain, boolean doDrain) {
		...
		for(IFluidTank handler : fluidTanks) {
			if(totalDrained == null) {
				...
				if(totalDrained != null) maxDrain -= totalDrained.amount;
			}
			else {
