 ...
 * @author 3TUSK
 */
public class RongFluidTank implements IFluidTank, IFluidHandler {
	...
	public RongFluidTank(int capacity) {
		this(capacity, "tank");
	}

	public RongFluidTank(int capacity, String tagName) {
	...
	public void writeToNBT(NBTTagCompound tag) {
		...
		if(fluidInv != null) tag.setTag(tankName, fluidInv.writeToNBT(newTag));
	}
	...
	public int fill(FluidStack resource, boolean doFill) {
		...
		if(this.fluidInv == null) {
			if(doFill) this.fluidInv = resource;
		...
		if(this.fluidInv.isFluidEqual(resource)) {
			...
			if(doFill) {
				if(newAmount > capacity) fluidInv.amount = capacity;
				else fluidInv.amount = newAmount;
			}
	...
	public FluidStack drain(int maxDrain, boolean doDrain) {
		...
		if(this.fluidInv.amount <= maxDrain) {
			...
			if(doDrain) {
				...
				if(fluidInv.amount <= 0) fluidInv = null;
			}
	...
	private void forceFillTank(FluidStack stack) {
		if(stack != null) this.fluidInv = stack.copy();
	}
