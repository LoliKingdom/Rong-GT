    ...
    public static void discoverAndLoadAnnotatedMaterialHandlers(ASMDataTable asmDataTable) {
        ...
        for(ASMData annotationData : annotations) {
            ...
            try {
                ...
                RTLog.logger.error("Failed to load annotated material handler class {}. FML annotation data is broken perhaps?", materialHandlerClassName);
                continue;
            }
            ...
            if(denyReason == null) {
                RTLog.logger.info("Registered material handler {}", materialHandler.getClass().getName());
                Material.registerMaterialHandler(materialHandler);
                materialHandlersRegistered++;
            } 
            else {
            	RTLog.logger.error("Failed to load material handler class {} from {}: {}",
                    materialHandlerClassName, annotationData.getCandidate().getModContainer().getName(), denyReason, loadingError);
            	RTLog.logger.error("Consult the mod author for fix on his side.");
            }
        }
        RTLog.logger.info("{} annotated material handlers registered", materialHandlersRegistered);
    }
