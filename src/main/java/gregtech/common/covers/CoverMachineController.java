...
public class CoverMachineController extends CoverBehavior implements CoverWithUI {
    ...
    public List<ControllerMode> getAllowedModes() {
        ...
        for (ControllerMode controllerMode : ControllerMode.values()) {
            ...
            if (controllerMode.side == null) {
                controllable = coverHolder.getCapability(GregtechCapabilities.CAPABILITY_CONTROLLABLE, null);
            } else {
                CoverBehavior coverBehavior = coverHolder.getCoverAtSide(controllerMode.side);
                if (coverBehavior != null) {
                    controllable = coverBehavior.getCapability(GregtechCapabilities.CAPABILITY_CONTROLLABLE, null);
                }
            }
    ...
    private IControllable getControllable() {
        ...
        if(side == null) {
            return coverHolder.getCapability(GregtechCapabilities.CAPABILITY_CONTROLLABLE, null);
        } else {
            CoverBehavior coverBehavior = coverHolder.getCoverAtSide(side);
            if(coverBehavior == null) {
                return null;
            }
            return coverBehavior.getCapability(GregtechCapabilities.CAPABILITY_CONTROLLABLE, null);
        }
    }
