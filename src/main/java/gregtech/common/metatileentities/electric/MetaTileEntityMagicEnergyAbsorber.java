...
public class MetaTileEntityMagicEnergyAbsorber extends TieredMetaTileEntity {
    ...
    private void findEssentiaAndDrain() {
    	EnumFacing facing = getHolder().getMetaTileEntity().getFrontFacing();
    	if(essentiaLeft == 0) {
    		for(Aspect aspect : Aspect.aspects.values()) {
    			if(EssentiaHandler.findEssentia(getHolder(), aspect, facing, 4, true)) {
    				int energy = calculateEssentiaToEU(aspect);
    				if(energyContainer.getEnergyCanBeInserted() >= energy) {
    					boolean drainedEssentia = EssentiaHandler.drainEssentia(getHolder(), aspect, facing, 4, 2);
    					if(drainedEssentia) {
    ...
    private int calculateEssentiaToEU(Aspect aspect) {
    	...
    	else if(aspect == Aspect.ENERGY || aspect == Aspect.MAGIC) {
			return 24;
		}
		else if(aspect == Aspect.AURA || aspect == Aspect.FLUX || aspect == Aspect.ELDRITCH) {
			return 48;
		}
		else {
			return 16;
