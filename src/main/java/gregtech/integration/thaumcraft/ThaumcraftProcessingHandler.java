	...
	public static void init() {
		...
		for(Material material : Material.MATERIAL_REGISTRY) {			
			if(material.hasFlag(DustMaterial.MatFlags.GENERATE_ORE)) {
				...
				ThaumcraftApi.addCrucibleRecipe(new ResourceLocation(GTValues.MODID, "redsandstone_metal_purification" + m.getLocalizedName().toLowerCase()), new CrucibleRecipe("METALPURIFICATION", OreDictUnifier.get(OrePrefix.cluster, m), "oreRedSandstone" + m.toCamelCaseString(), new AspectList().merge(Aspect.METAL, 5).merge(Aspect.ORDER, 5)));
